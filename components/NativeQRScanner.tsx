...
export default function NativeQRScanner() {
  ...
  const { isNative, scheduleNotification } = useNativeCapabilities();
            ...
            {scanning ? (
              <CameraView
                ...
                onBarcodeScanned={(e) => {
                  ...
                  lock.current = true;
                  setScanning(false);
                  setResult(e.data || "BC2U-CUP-001"); // mock fallback
                  scheduleNotification("Scan Complete", "QR detected", 0).catch(()=>{});
                }}
              />
            ) : (
              <Text style={{ color: "#2563EB", fontWeight: "700" }}>Tap scan</Text>
            )}
          </View>

          <Button onPress={startScan} style={{ marginTop: 12 }}>
            <Text style={{ color: "#fff", fontWeight: "700" }}>{scanning ? "Scanning..." : "Scan QR Code"}</Text>
          </Button>
