...
export default function CustomerCo2ReportScreen() {
  ...
  const loadSummaryData = useCallback(async () => {
    try {
      ...
      if (response.statusCode === 200) {
        ...
        // Filter by business (client-side since API might not support it)
        if (businessFilter !== 'all') {
          items = items.filter((item: TransactionItem) => 
            item.businessId?._id === businessFilter
          );
        }
  ...
  }, [statusFilter, fromDate, toDate, productName, businessFilter]);
  ...
  }, [statusFilter, fromDate, toDate, productName, businessFilter, limit]);
  ...
  const uniqueBusinesses = useMemo(() => {
    ...
    // Get businesses from summary data (all transactions)
    summaryData.forEach((item) => {
      if (item.businessId?._id && item.businessId?.businessName) {
        if (!businessMap.has(item.businessId._id)) {
          businessMap.set(item.businessId._id, {
            id: item.businessId._id,
            name: item.businessId.businessName,
          });
        }
      }
    });
    
    return Array.from(businessMap.values()).sort((a, b) => 
  ...
  const renderTransactionCard = ({ item }: { item: TransactionItem }) => {
    ...
    const businessName = item.businessId?.businessName || 'Unknown Business';
    ...
    return (
      <View style={styles.filterSection}>
        ...
        {uniqueBusinesses.length > 0 && (
          ...
          >
            <TouchableOpacity
              style={[styles.filterChip, businessFilter === 'all' && styles.filterChipActive]}
              onPress={() => setBusinessFilter('all')}
              activeOpacity={0.7}
            >
              <Text style={[styles.filterChipText, businessFilter === 'all' && styles.filterChipTextActive]}>
                All Businesses
              </Text>
            </TouchableOpacity>
            {uniqueBusinesses.map((business) => (
              <TouchableOpacity
                key={business.id}
                style={[styles.filterChip, businessFilter === business.id && styles.filterChipActive]}
                onPress={() => setBusinessFilter(business.id)}
                activeOpacity={0.7}
              >
                <Text 
                  style={[styles.filterChipText, businessFilter === business.id && styles.filterChipTextActive]}
                  numberOfLines={1}
                >
                  {business.name}
                </Text>
