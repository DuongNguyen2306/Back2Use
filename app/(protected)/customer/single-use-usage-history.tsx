...
import { SingleUseProductUsage, singleUseProductUsageApi } from "../../../src/services/api/businessService";
...
export default function CustomerSingleUseUsageHistory() {
  const { state } = useAuth();
  ...
  useEffect(() => {
    if (state.isHydrated && state.accessToken && state.isAuthenticated && state.role === 'customer') {
      loadUserData();
  ...
  const loadUsageHistory = async (pageNum: number, isRefreshing: boolean = false) => {
    try {
      ...
      const enrichedUsages: UsageHistoryItem[] = usagesArray.map((usage: any) => {
        return {
          ...usage,
          product: usage.product || usage.singleUseProductId,
          co2PerUnit: usage.co2PerUnit || usage.co2Emission || 0,
        };
      });
      ...
      if (pageNum === 1) {
        setUsageHistory(enrichedUsages);
      } else {
        setUsageHistory(prev => [...prev, ...enrichedUsages]);
      }

      setHasMore(enrichedUsages.length >= 20);
      setPage(pageNum);
  ...
  const getBusinessName = (item: UsageHistoryItem) => {
    return item.borrowTransaction?.businessId?.businessName || 'N/A';
  };
  ...
  const getTxHash = (item: UsageHistoryItem) => {
    return item.blockchainTxHash || null;
  };

  const handleItemPress = (item: UsageHistoryItem) => {
    if (item.borrowTransactionId) {
      router.push(`/(protected)/customer/transaction-detail/${item.borrowTransactionId}`);
    }
  };
        ...
        <View style={styles.cardHeaderContent}>
          ...
          {getTxHash(item) && (
            <View style={styles.txHashRow}>
              ...
              <Text style={styles.fullTxHashText} numberOfLines={1}>{getTxHash(item)}</Text>
            </View>
          )}
        </View>
      </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.titleSection}>
          ...
          <Text style={styles.subtitle}>Your single-use product usage log.</Text>
        </View>
